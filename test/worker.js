...
var targ = new Uint8Array(1024 * 1024 * 100);
// for (let i = 0; i < 1024 * 1024 * 100; i++) {
//     targ[i] = 88; // "x"
// }
// var targ = new String(temp);


const handler = {
    get(target, prop, receiver) {
        self.postMessage({console: String(prop)});
        if (String(prop) === "Symbol(Symbol.toPrimitive") {
            return Uint8Array
        }
        if (prop === "toString") {
            self.postMessage({console: target.toString()});
            return () => {return }
        }
        if (prop === "valueOf") {
            self.postMessage({console: typeof(target.valueOf())});
            return targ
        }
        return Reflect.get(...arguments);
    },
    getPrototypeOf(target) {
        self.postMessage({console: "called proto"});
        return target.getPrototypeOf();
    }
};

const BUFFER = new Proxy(targ, handler);
self.postMessage({console: "1: " + String(typeof(new Uint8Array(1024 * 1024 * 100)))});
